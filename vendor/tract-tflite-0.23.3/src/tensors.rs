...
pub fn flat_tensor_to_tract_fact<'m>(
    ...
    let concrete_shape = if let Some(shape) = flat.shape().or_else(|| flat.shape_signature()) {
        let mut concrete_shape = Vec::with_capacity(shape.len());
        for dim in shape {
            ensure!(
                dim >= 0,
                "Dynamic TFLite tensor shape is not supported for tensor {id} ({})",
                flat.name().unwrap_or("<unnamed>")
            );
            concrete_shape.push(dim as usize);
        }
        concrete_shape
    } else {
        vec![]
    };
    let mut fact = dt.fact(concrete_shape);
    ...
    Ok((fact, flat.name().unwrap_or("<unnamed>")))
}
