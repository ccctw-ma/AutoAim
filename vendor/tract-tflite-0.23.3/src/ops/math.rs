...
pub fn register_all(reg: &mut Registry) {
    ...
    reg.reg_to_tract(BuiltinOperator::FLOOR_DIV, deser_floor_div);
...
fn wire_cast_and_rank_broadcast(op: &mut DeserOp) -> TractResult<TVec<OutletId>> {
    let facts = op.facts()?;
    let operating_datum_type = if facts.iter().any(|fact| fact.datum_type.is_quantized()) {
        DatumType::F32
    } else {
        DatumType::super_type_for(facts.iter().map(|f| f.datum_type)).context("No super type")?
    };
    let wire = wire_cast(
...
fn deser_floor_div(op: &mut DeserOp) -> TractResult<TVec<OutletId>> {
    let wires = wire_cast_and_rank_broadcast(op)?;
    op.ctx.target.wire_node(op.prefix, tract_core::ops::math::div(), &wires)
}
