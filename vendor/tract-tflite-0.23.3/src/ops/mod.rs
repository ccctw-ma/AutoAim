...
pub fn register_all(reg: &mut Registry) {
    ...
    reg.reg_to_tract(BuiltinOperator::DEQUANTIZE, de_cast);
    reg.reg_to_tract(BuiltinOperator::QUANTIZE, de_cast);
...
fn de_cast(op: &mut DeserOp) -> TractResult<TVec<OutletId>> {
    let dt: DatumType = if let Some(options) = op.flat.builtin_options_as_cast_options() {
        options.out_data_type().try_into()?
    } else {
        op.output_facts[0].datum_type
    };
    op.ctx.target.wire_node(op.prefix, cast(dt), op.inputs)
}
...
fn linearops_quantization_suport(
    ...
    if op.output_facts[0].datum_type.is_quantized() && input.datum_type.is_quantized() {
        let p = &op.prefix;
