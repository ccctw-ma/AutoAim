...
pub fn register_all(reg: &mut Registry) {
    ...
    reg.reg_to_tract(BuiltinOperator::ARG_MAX, de_arg_max);
...
fn de_arg_max(op: &mut DeserOp) -> TractResult<TVec<OutletId>> {
    let (_input, axis) = args_2!(op.facts()?);
    let axis = axis.konst.as_ref().context("Dynamic ARG_MAX axis is not supported")?;
    let axis = axis.cast_to_scalar::<i32>()?;
    let input_rank = op.ctx.target.outlet_fact(op.inputs[0])?.rank();
    let axis = if axis < 0 { axis + input_rank as i32 } else { axis } as usize;
    op.ctx.target.wire_node(
        op.prefix,
        Reduce {
            axes: tvec![axis],
            reducer: Reducer::ArgMax(false),
        },
        &[op.inputs[0]],
    )
}
