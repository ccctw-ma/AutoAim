...
fn de_conv2d(op: &mut DeserOp) -> TractResult<TVec<OutletId>> {
    ...
    if input.datum_type.is_float() {
        inputs[1] = op.ctx.target.wire_node(
            format!("{}.cast_kernel", op.prefix),
            cast(input.datum_type),
            &[inputs[1]],
        )?[0];
...
fn de_dw_conv2d(op: &mut DeserOp) -> TractResult<TVec<OutletId>> {
    ...
    if input.datum_type.is_float() {
        inputs[1] = op.ctx.target.wire_node(
            op.ctx.target.unique_name(format!("{}.kernel", op.prefix)),
            cast(input.datum_type),
            &[inputs[1]],
        )?[0];
        inputs[2] = op.ctx.target.wire_node(
    ...
    } else if bias.datum_type.is_quantized() {
        inputs[2] = op.ctx.target.wire_node(
            op.ctx.target.unique_name(format!("{}.bias", op.prefix)),
